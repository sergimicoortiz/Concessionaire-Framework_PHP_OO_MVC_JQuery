function ajaxForSearch(url, f_data = undefined) {
    ajaxPromise(friendlyURL(url), 'POST', 'JSON', f_data)
        .then(function (data) {
            if (data.length == 0) {
                ...
                var box = true;
                data.forEach(car => {
                    const img = window.location.origin + '/Concessionaire-Framework_PHP_OO_MVC_JQuery/view/img/' + car.img.split('/')[3]
                    //console.log(img);
                    map_list(car, img);
                    var id = car['car_id'];
            ...
            var callback = friendlyURL('?module=error&op=view&param=503&param2=shop_list_error_ajax');
...
function loadCars(offset = 0, limit = 8) {
    ...
    switch (option) {
        case 'filters':
            ajaxForSearch('?page=shop&op=list_cars_filters', { "f_data": filters, "offset": offset, "limit": limit });
            break;

        case 'all':
            ajaxForSearch('?page=shop&op=list_cars', { "offset": offset, "limit": limit });
            break;
    }//end swich
    loadOut();
...
function printfilters() {
    ...
    ajaxPromise(friendlyURL('?page=shop&op=list_brands'), 'GET', 'JSON')
        .then(function (data) {
            data.forEach(brand_tmp => {
                var brand = brand_tmp.brand_name;
                if (brand == f_brand) {
            ...
            var callback = friendlyURL('?module=error&op=view&param=503&param2=shop_brand_error_ajax');
            window.location.href = callback;
        })//end ajaxPrimise
    ...
    ajaxPromise(friendlyURL('?page=shop&op=list_category'), 'GET', 'JSON')
        .then(function (data) {
            data.forEach(category_tmp => {
                var category = category_tmp.category_name;
            ...
            var callback = friendlyURL('?module=error&op=view&param=503&param2=shop_category_error_ajax');
            window.location.href = callback;
        })//end ajaxPrimise
    ...
    ajaxPromise(friendlyURL('?page=shop&op=list_fuels'), 'GET', 'JSON')
        .then(function (data) {
            data.forEach(fuel_tmp => {
                var fuel = fuel_tmp.fuel_type_name;
                if (fuel == f_fuel) {
            ...
            var callback = friendlyURL('?module=error&op=view&param=503&param2=shop_fuel_error_ajax');
            window.location.href = callback;
        })//end ajaxPrimise
    ...
    ajaxPromise(friendlyURL('?page=shop&op=list_city'), 'GET', 'JSON')
        .then(function (data) {
            data.forEach(city_tmp => {
                var city = city_tmp.city;
                if (city == f_city) {
            ...
            var callback = friendlyURL('?module=error&op=view&param=503&param2=shop_city_error_ajax');
            window.location.href = callback;
        })//end ajaxPrimise
    $('<br>').appendTo('#filter_form')
...
function load_pagination(limit = 8) {
    ...
    ajaxPromise(friendlyURL('?page=shop&op=count_cars'), 'POST', 'JSON', { "filters": filters })
        .then(function (data) {
            ...
            var callback = friendlyURL('?module=error&op=view&param=503&param2=shop_pagination_error_ajax');
            window.location.href = callback;
        })//end ajaxPromise img
}//end load pagination
